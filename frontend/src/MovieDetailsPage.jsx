    ...
    return (
        ...
        >
            ...
            <div style={{ padding: '0 20px 20px', position: 'relative', top: '-20px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', color: '#64748b', fontSize: '0.85rem', marginBottom: '16px' }}>
                    <span style={{ color: '#10b981', fontWeight: 'bold' }}>{displayMovie.rating ? Math.round(displayMovie.rating * 10) : 95}% Match</span>
                    <span>{displayMovie.year || '2022'}</span>
                    <span>{displayMovie.genre}</span>
                    <span style={{ border: '1px solid #cbd5e1', padding: '0 4px', fontSize: '0.7rem', color: '#64748b', borderRadius: '3px' }}>HD</span>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '20px' }}>
                    <motion.button
                        ...
                        style={{
                            ...
                            gap: '8px',
                            cursor: 'pointer',
                            boxShadow: '0 4px 12px rgba(255, 10, 22, 0.25)'
                        }}
                    >
                        <>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-around', marginBottom: '24px' }}>
                    <ActionButton
                        icon={myList && myList.find(m => (m._id || m.id) == (displayMovie._id || displayMovie.id)) ? <Check size={24} color="#10b981" /> : <Plus size={24} />}
                    ...
                    />
                    <ActionButton
                        icon={<ThumbsUp size={24} fill={likedVideos && likedVideos.find(m => (m._id || m.id) == (displayMovie._id || displayMovie.id)) ? "#ff0a16" : "none"} color={likedVideos && likedVideos.find(m => (m._id || m.id) == (displayMovie._id || displayMovie.id)) ? "#ff0a16" : "currentColor"} />}
