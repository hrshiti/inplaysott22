...
const SearchPage = ({ onMovieClick }) => { // Remove allContent prop as we fetch it
    ...
    return (
        <div className="app-container" style={{ minHeight: '100vh', background: '#ffffff', paddingBottom: '20px' }}>
            {/* Search Header */}
            <div style={{
                position: 'sticky',
                top: 0,
                ...
                <div style={{
                    ...
                    {query && (
                        <button
                            onClick={() => setQuery('')}
                            style={{ background: 'transparent', border: 'none', color: '#64748b', padding: 0, display: 'flex' }}
                        >
                            <X size={18} />
                        </button>
                    )}
                        ...
                        {results.map(item => (
                            ...
                            >
                                ...
                                <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                    <h3 style={{ fontSize: '0.95rem', fontWeight: 600, color: '#0f172a', lineHeight: '1.2' }}>
                                        {item.title}
                                    </h3>
                    ...
                    <div style={{ marginTop: '20px' }}>
                        {!query && (
                            <>
                                ...
                                {history.length > 0 && (
                                    <div style={{ marginBottom: '32px' }}>
                                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
                                            <h3 style={{ color: '#0f172a', fontSize: '1rem', fontWeight: '600' }}>Recent Searches</h3>
                                            <button
                                                onClick={() => {
                                                    setHistory([]);
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column' }}>
                                            {history.map((term, index) => (
                                                ...
                                                >
                                                    <Clock size={16} color="#64748b" style={{ marginRight: '16px' }} />
                                                    <span style={{ color: '#0f172a', flex: 1, fontSize: '0.95rem' }}>{term}</span>
                                                    <button
                                ...
                                <div>
                                    <h3 style={{ color: '#0f172a', fontSize: '1rem', fontWeight: '600', marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                                        <TrendingUp size={18} color="#ff0a16" />
                                        Most Searched
                                    </h3>
